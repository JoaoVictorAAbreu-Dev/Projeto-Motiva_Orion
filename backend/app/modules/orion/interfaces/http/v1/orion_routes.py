...
from app.modules.orion.domain.schemas import (
    ...
    PlanoSemanalRead,
    SentinelNDVIRequest,
    SentinelNDVIStatsRead,
    TrechoRead,
)
from app.modules.orion.infrastructure.persistence.core_repositories import IndicadorRepository, IntervencaoRepository, MissaoRepository, TrechoRepository
...
@router.post('/satellite/ndvi/stats', response_model=SentinelNDVIStatsRead)
async def satellite_ndvi_stats(
    payload: SentinelNDVIRequest,
    _: UsuarioModel = Depends(require_roles('admin', 'gestor', 'coordenador')),
    service: SentinelSatelliteService = Depends(get_sentinel_satellite_service),
) -> SentinelNDVIStatsRead:
    if len(payload.bbox) != 4:
        raise HTTPException(status_code=400, detail='bbox must have 4 numeric values')
    try:
        ndvi_mean = await service.get_ndvi_mean(
            bbox=payload.bbox,
            date_from=payload.date_from,
            date_to=payload.date_to,
            max_cloud_coverage=payload.max_cloud_coverage,
        )
        return SentinelNDVIStatsRead(ndvi_mean=ndvi_mean)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except RuntimeError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail='Sentinel NDVI stats request failed') from exc
