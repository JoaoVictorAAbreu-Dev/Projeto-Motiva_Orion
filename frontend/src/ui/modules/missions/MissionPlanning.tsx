...
export function MissionPlanning({ missions }: Props) {
  return (
    <section className="rounded-xl border border-slate-700 bg-slate-900/80 p-4 shadow-panel">
      <h2 className="mb-3 text-base font-semibold text-white">Planejador de Missoes</h2>
        ...
        {missions.map((mission) => (
          <article key={mission.id} className="rounded-md border border-slate-700 bg-slate-800/70 p-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-white">Missao {mission.codigo}</p>
              <span className="rounded bg-critical/20 px-2 py-1 text-xs font-semibold text-critical">Prioridade {mission.prioridade}</span>
            </div>
            <p className="mt-2 text-sm text-slate-300">Trechos: {mission.trecho_ids.join(', ')}</p>
            <p className="text-sm text-slate-300">Equipe sugerida: {mission.equipe}</p>
